...
from .models import Category, Product, ProductImage, ProductView, ProductInquiry
...
class CategoryAdmin(ModelAdmin):
    list_display = ['name', 'category_type', 'get_image_preview', 'is_active', 'product_count', 'created_at']
...
class ProductAdmin(ModelAdmin):
    list_display = [
        'get_main_image_preview', 'title', 'brand', 'model', 'year', 
        'category', 'formatted_price', 'location_city', 'seller', 
        'is_active', 'is_sold', 'view_count', 'contact_link'
    ]
    list_filter = [
        'category', 'condition', 'transmission', 'fuel_type', 
        'is_active', 'is_sold', 'is_featured', 'created_at',
        'location_province'
    ]
    ...
    fieldsets = [
        ...
        ('Detail Kendaraan', {
            'fields': [
                ('brand', 'model', 'year'),
                ('condition', 'color'),
                ('engine_capacity', 'transmission', 'fuel_type'),
                'mileage',
                'features'
            ]
        }),
    ...
    def get_main_image_preview(self, obj):
        ...
        return format_html('<div style="width: 60px; height: 45px; background: #f0f0f0; border-radius: 4px; display: flex; align-items: center; justify-content: center; font-size: 12px; color: #666;">No Image</div>')
...
class ProductViewAdmin(ModelAdmin):
    ...
    readonly_fields = ['product', 'ip_address', 'user_agent', 'viewed_at']
...
class ProductInquiryAdmin(ModelAdmin):
    list_display = ['product', 'inquirer_name', 'inquirer_phone', 'created_at']
    ...
    fieldsets = [
        ('Informasi Inquiry', {
            'fields': ['product', 'inquirer_name', 'inquirer_phone']
        }),
        ('Pesan', {
            'fields': ['message']
        }),
